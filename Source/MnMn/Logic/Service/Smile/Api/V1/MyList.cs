...
namespace ContentTypeTextNet.MnMn.MnMn.Logic.Service.Smile.Api.V1
{
    ...
    {
        public MyList(Mediator mediator)
        ...
        public async Task<IList<SmileVideoSearchMyListFinderViewModel>> SearchPage(string query, int pageNumber)
        {
            ...
            using(var page = new PageLoader(Mediator, SessionBase, SmileMediatorKey.mylistSearch, ServiceType.Smile)) {
                ...
                foreach(var baseElement in baseElements) {
                    var paragraphElements = baseElement.SelectNodes(".//p")?.ToEvaluatedSequence();
                    ...
                    var finder = new SmileVideoSearchMyListFinderViewModel(Mediator, myListId, myListName, myListItemCount, query, totalItemCountValue, showCount);
                    result.Add(finder);
                }
        ...
        async Task<SmileJsonResultModel> RequestPost(PageLoader page)
        {
            ...
            Mediator.Logger.Debug(result.Status.ToString());
            return result;
        }
