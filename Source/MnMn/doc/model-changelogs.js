				...
				'logs': [
					{
						...
						'comments': [
							...
							'これは次回更新時にも(憶えてたら)書くけど上手くいかない場合は手動でダウンロードか旧処理を使用する設定にしてください',
				...
				'logs': [
					...
					},
					{
						...
						'comments': [
							...
							'今回分では展開処理は既存の Updater で使ってる処理をほぼベタ移植した(コマンドライン関係以外)。たとえ部分的にでも私はゼロベースで書き直したスマートで美しくて素晴らしいコードより既に動いているゲロゲロなプログラムがもたらす実績ありっていう最強の信頼度を優先するよ',
							'歳食ったら保守的になるのだ',
							'移植したって言ってもいろいろうところはあるからヤバいときは手動か App.config 書き換えて既存処理使ってね',
